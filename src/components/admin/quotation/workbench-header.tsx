...
export function WorkbenchHeader({ title, description, children }: { title: string; description?: string; children?: React.ReactNode }) {
  return <header className="space-y-6">
    <nav aria-label="报价工作台" className="flex flex-wrap gap-2 border-b pb-4 text-sm">
      {[['/admin/sales-quotations', '报价单'], ['/admin/business-customers', '客户列表'], ['/admin/quotation-products', '常用产品'], ['/admin/quotation-settings', '报价设置']].map(([href, name]) => <Link key={href} href={href} className="rounded-full px-4 py-2 font-medium text-slate-600 transition-colors hover:bg-slate-100 hover:text-slate-950">{name}</Link>)}
    </nav>
