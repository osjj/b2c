...
export function QuotationSettingsForm({ initialValue }: { initialValue: WorkbenchSettings }) {
  ...
  return <form className="max-w-4xl space-y-6" onSubmit={(event) => { event.preventDefault(); startTransition(async () => { setError(''); setMessage(''); try { const result = await saveQuotationSettings(value); if (!result.success) { setError(result.reason); return } setMessage(result.reason) } catch { setError('设置保存失败，请重试') } }) }}>
    <fieldset disabled={busy || uploading} className="space-y-6"><section className="space-y-5 rounded-2xl border bg-white p-6"><h2 className="font-semibold">公司抬头</h2>{([['companyName', '公司名称'], ['contactLine', '联系方式（右侧）'], ['address', '地址（左侧）'], ['website', '网站（右侧）'], ['email', '邮箱（右侧）'], ['tagline', '页脚标语']] as const).map(([key, label]) => <label key={key} className="block space-y-2 text-sm">{label}<Input value={value.brand[key] ?? ''} type={key === 'email' ? 'email' : 'text'} onChange={(e) => setValue({ ...value, brand: { ...value.brand, [key]: e.target.value } })} /></label>)}<p className="text-xs text-slate-500">地址左对齐；联系方式、网站、邮箱右对齐。原来合并填写的内容可手动拆到对应字段。</p></section>
    <section className="grid gap-6 rounded-2xl border bg-white p-6 md:grid-cols-2">{([['logoSourceId', '公司 Logo'], ['sealSourceId', '公司印章']] as const).map(([key, label]) => <div key={key} className="space-y-3"><h2 className="font-semibold">{label}</h2><p className="text-xs text-slate-500">可选，不超过 1 MB，建议使用透明背景 PNG。只保留一张。</p><SimpleImagePicker maxImages={1} maxSizeMb={1} label={`上传${label}`} images={value[key] ? [{ id: value[key], kind: 'source', name: label }] : []} onChange={(images) => { const id = images.at(-1)?.id ?? null; setValue({ ...value, [key]: id, ...(key === 'sealSourceId' ? { useSeal: Boolean(id) } : {}) }) }} onBusy={setUploading} disabled={busy} /></div>)}<label className="flex items-center gap-2 text-sm md:col-span-2"><input type="checkbox" checked={value.useSeal} onChange={(e) => setValue({ ...value, useSeal: e.target.checked })} />正式 PDF 和 Excel 使用印章，并叠加报价日期</label><p className="text-xs text-slate-500 md:col-span-2">上传新印章后自动勾选；请先保存设置。新生成的正式文件使用当前已保存的 Logo 和印章设置。预览 PDF 不盖章；已生成文件不会被覆盖，需创建修订版再生成。</p>{value.sealSourceId && !value.useSeal && <p className="text-sm text-amber-800 md:col-span-2">已上传印章但未启用，正式文件不会盖章。需要盖章请勾选上方选项。</p>}</section>
