...
export default async function CommonProductPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const price = commonProductDefaultsSchema.parse(defaults?.value ?? {})
  return <div className="mx-auto max-w-5xl space-y-8"><WorkbenchHeader title="编辑常用产品" description="更新默认资料，不影响任何已保存的报价内容。" /><CommonProductForm key={product.updatedAt.toISOString()} initialValue={{ id, name: product.nameEn || product.nameZh || '', specifications: z.array(z.string()).catch([]).parse(product.specifications).join('\n'), unit: product.unit, ...price, description: price.description ?? '', active: product.status !== 'INACTIVE', expectedUpdatedAt: product.updatedAt.toISOString() }} /><CommonProductImages key={`images-${product.updatedAt.toISOString()}`} productId={id} updatedAt={product.updatedAt.toISOString()} images={product.images.flatMap((image) => image.sourceFileId ? [{ id: image.sourceFileId, kind: 'source' as const, name: image.displayName || '产品图片' }] : [])} /></div>
}
