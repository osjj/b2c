 ...
 * 2. 准备 scripts/data.json 文件
 * 3. 运行: npx tsx scripts/batch-upload-products.ts
 *
 * 或者指定自定义文件:
 * npx tsx scripts/batch-upload-products.ts ./my-products.json
 */
...
async function loadData(filePath: string): Promise<Product[]> {
...
async function uploadBatch(products: Product[]): Promise<BatchResult[]> {
  const response = await fetch(`${API_URL}/api/admin/products/batch`, {
    method: 'POST',
    headers: {
...
async function main() {
  ...
  try {
    ...
    for (let i = 0; i < totalBatches; i++) {
      ...
      console.log(`📤 上传批次 ${i + 1}/${totalBatches} (${batch.length} 个产品)...`)
      ...
      try {
        ...
        batch.forEach(p => errors.push({ slug: p.slug, error: 'Batch upload failed' }))
      }
